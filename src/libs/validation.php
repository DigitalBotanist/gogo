 <!-- references: https://www.phptutorial.net/php-tutorial/php-validation/ -->
...
// default error messages for each validation error
...
// validating input data and return errors if validation failed 
function validate(array $data, array $fields, array $messages = []): array {

    // function to split the values in fields variable 
    ...
    // validating each value in the $data variable one by one  
            ...
            if(is_callable($fn)) {
                ...
                if (!$pass) {
                    $errors[$field] = sprintf(
                        $messages[$field][$rule] ?? $validation_errors[$rule_name], 
                        $field, 
                        ...$params
                    ); 
                }
...
function is_unique(array $data, string $field, string $table, string $column): bool {
    ...
    $sql = "SELECT $column FROM $table WHERE $column = '$data[$field]';"; 
